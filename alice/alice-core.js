...
(function (root) {
  ...
  var UNDEF_CH = "\uFFFD";
  var NT = ALICE_TRAILS.length;      // 188
  var LEAD_IDX = new Int16Array(256); LEAD_IDX.fill(-1);
  var TRAIL_IDX = new Int16Array(256); TRAIL_IDX.fill(-1);
  for (var i = 0; i < ALICE_LEADS.length; i++) LEAD_IDX[ALICE_LEADS[i]] = i;
  for (var j = 0; j < ALICE_TRAILS.length; j++) TRAIL_IDX[ALICE_TRAILS[j]] = j;

  var utf8Dec = new TextDecoder("utf-8");

  // ---- UTF-8 编码（精确等价 Python 的 errors="surrogatepass"）----
  // 原生 TextEncoder 会把「孤立代理项」替换成 U+FFFD（EF BF BD），
  // 而 Python 的 surrogatepass 会把它按代理项本身的码位编成 3 字节。
  // 正常文本走原生快速路径（零额外开销），只有出现孤立代理项时才走慢路径。
  var utf8EncNative = new TextEncoder();
  function utf8Encode(text) {
    var slow = false, i, c, n;
    for (i = 0; i < text.length; i++) {
      c = text.charCodeAt(i);
      if (c >= 0xD800 && c <= 0xDBFF) {
        n = i + 1 < text.length ? text.charCodeAt(i + 1) : 0;
        if (n >= 0xDC00 && n <= 0xDFFF) { i++; continue; }   // 合法代理对，交给原生
        slow = true; break;
      }
      if (c >= 0xDC00 && c <= 0xDFFF) { slow = true; break; } // 孤立低位代理
    }
    if (!slow) return utf8EncNative.encode(text);

    var out = [];
    for (var k = 0; k < text.length; k++) {
      var u = text.charCodeAt(k);
      if (u >= 0xD800 && u <= 0xDBFF && k + 1 < text.length) {
        var nx = text.charCodeAt(k + 1);
        if (nx >= 0xDC00 && nx <= 0xDFFF) {
          var cp = 0x10000 + ((u - 0xD800) << 10) + (nx - 0xDC00);
      ...
      else out.push(0xE0 | (u >>> 12), 0x80 | ((u >>> 6) & 0x3F), 0x80 | (u & 0x3F));
    }
    return new Uint8Array(out);
  }

  // ---- 解码：字节流 -> 字符串（严格按 Python cp932 规则）----
  function cp932Decode(bytes) {
    var out = [];
    var n = bytes.length;
    var k = 0;
    while (k < n) {
      var b = bytes[k];
      if (b < 0x80) { out.push(String.fromCharCode(b)); k++; continue; }
      if (b === 0x80) { out.push("\u0080"); k++; continue; }
      if (b >= 0xA1 && b <= 0xDF) { out.push(String.fromCharCode(0xFF61 + b - 0xA1)); k++; continue; }
      if (b === 0xA0) { out.push("\uF8F0"); k++; continue; }
      if (b === 0xFD) { out.push("\uF8F1"); k++; continue; }
      if (b === 0xFE) { out.push("\uF8F2"); k++; continue; }
      if (b === 0xFF) { out.push("\uF8F3"); k++; continue; }
      var li = LEAD_IDX[b];
      if (li >= 0 && k + 1 < n) {
        var ti = TRAIL_IDX[bytes[k + 1]];
        if (ti >= 0) {
          var ch = ALICE_DBL.charAt(li * NT + ti);
          if (ch !== UNDEF_CH) { out.push(ch); k += 2; continue; }
        }
      }
      out.push(UNDEF_CH); k++;
    }
  ...
  // ---- 编码：字符 -> 字节序列（首现优先，已与 Python 编码表逐字符校验一致）----
  var encMap = null;
  function buildEncodeMap() {
    if (encMap) return encMap;
    var m = new Map();
    for (var li = 0; li < ALICE_LEADS.length; li++) {
      var base = li * NT;
      var L = ALICE_LEADS[li];
      for (var ti = 0; ti < NT; ti++) {
        var ch = ALICE_DBL.charAt(base + ti);
        if (ch !== UNDEF_CH && !m.has(ch)) m.set(ch, [L, ALICE_TRAILS[ti]]);
      }
    }
    // 单字节区（优先级更高，覆盖任何潜在冲突）
    for (var b = 0; b < 0x80; b++) m.set(String.fromCharCode(b), [b]);
    m.set("\u0080", [0x80]);
    for (var h = 0xA1; h <= 0xDF; h++) m.set(String.fromCharCode(0xFF61 + h - 0xA1), [h]);
    m.set("\uF8F0", [0xA0]);
    m.set("\uF8F1", [0xFD]);
    m.set("\uF8F2", [0xFE]);
    m.set("\uF8F3", [0xFF]);
    // cp932 的 6 处非对称映射：这些字符可编码到某字节对，但该字节对解码回来是另一个字符
    // （Python 编码表特有行为，反转正向表无法得到，需显式补上）
    m.set("\u00A2", [0x81, 0x91]);
    m.set("\u00A3", [0x81, 0x92]);
    m.set("\u00AC", [0x81, 0xCA]);
    m.set("\u2016", [0x81, 0x61]);
  ...
  function encodeChar(ch) {
    var m = buildEncodeMap();
    var v = m.get(ch);
    return v || null;
  }

  // ---- 对外：正常文字 -> 爱丽丝语 ----
  function toAlice(text) {
    var bytes = utf8Encode(text);
    var alice = cp932Decode(bytes);
    var lost = 0;
    for (var i = 0; i < alice.length; i++) if (alice.charCodeAt(i) === 0xFFFD) lost++;
    return { result: alice, lost: lost };
  }

  // ---- 对外：爱丽丝语 -> 正常文字 ----
  // 注意：必须按「码位」遍历而不是按 charAt 的 UTF-16 码元，否则增补平面字符
  // （如 emoji）会被拆成两个代理项，多算一次丢失 —— Python 的 for ch in text
  // 是按码位来的，两者必须一致。
  function fromAlice(alice) {
    var bytes = [];
    var lost = 0;
    var i = 0;
    while (i < alice.length) {
      var ch = String.fromCodePoint(alice.codePointAt(i));
      i += ch.length;
      if (ch === "\uFFFD") { lost++; continue; }   // 翻译阶段就丢掉的占位符
      var enc = encodeChar(ch);
      if (enc) { for (var j = 0; j < enc.length; j++) bytes.push(enc[j]); }
      else lost++;
    }
    return { result: utf8Dec.decode(new Uint8Array(bytes)), lost: lost };
  }

  var api = { toAlice: toAlice, fromAlice: fromAlice, cp932Decode: cp932Decode,
              buildEncodeMap: buildEncodeMap, utf8Encode: utf8Encode };
...
(function (root) {
  ...
  var VERSION = "1.1.0";
  var core = root.AliceCore || null;
  if (!core) throw new Error("AliceTranslate: 缺少 AliceCore（请先加载码表与核心算法）");

  var ALIAS_FROM = { from: 1, decode: 1, back: 1, reverse: 1, "backward": 1, unalice: 1 };

  function s(v) { return v == null ? "" : String(v); }

  function normMode(mode) {
    var m = s(mode || "to").toLowerCase();
    return ALIAS_FROM[m] ? "from" : "to";
  }

  function pack(mode, input, r) {
    return {
      ok: true,
      api: "alice-translator",
      version: VERSION,
      mode: mode,                   // "to" = 正常文字→爱丽丝语, "from" = 爱丽丝语→正常文字
      input: input,
      result: r.result,
      lost: r.lost,                 // 无法映射、永久丢失的字符数
      lossless: r.lost === 0
    };
  }

  // 通用入口：translate(text, mode) -> 完整结果对象
  function translate(text, mode) {
    var m = normMode(mode);
    var t = s(text);
    return m === "from" ? pack("from", t, core.fromAlice(t)) : pack("to", t, core.toAlice(t));
  }
  ...
  var API = {
    ...
    // —— 完整结果 ——
    translate: translate,
    toAlice: function (t) { return translate(t, "to"); },
    fromAlice: function (t) { return translate(t, "from"); },
    // —— 只要字符串 ——
    to: function (t) { return translate(t, "to").result; },
    from: function (t) { return translate(t, "from").result; },
    // —— 只要丢失字符数 ——
    lossOf: function (t, mode) { return translate(t, mode).lost; },
    // —— 底层 ——
    core: core
  };

  root.AliceTranslate = API;
  if (typeof module !== "undefined" && module.exports && !module.exports.AliceTranslate) {
    module.exports.AliceTranslate = API;
  }
})(typeof globalThis !== "undefined" ? globalThis : this);
